...
def readLangs(lang1, lang2, path, reverse=False):
    ...
    pairs = [[normalizeString(s) for s in l.split('\t') if len(s)>1] for l in tqdm(lines.split('\n'))]
    pairs=[pair for pair in tqdm(pairs) if len(pair)>1 ]
...
def tensorFromSentence(lang, sentence, MAX_LENGTH=340):
